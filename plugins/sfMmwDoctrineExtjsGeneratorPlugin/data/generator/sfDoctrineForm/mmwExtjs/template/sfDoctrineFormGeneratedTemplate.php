...
<?php if ($relation->getAssociationTable()->hasField('mmw_sorting')): ?>
    if (isset($this->widgetSchema['<?php echo $this->underscore($relation['alias']) ?>_list']))
    {
      $defaults = Doctrine_Query::create()
        ->select('rel.<?php echo $relation['foreign'] ?>')
        ->from('<?php echo $relation->getAssociationTable()->getComponentName(); ?> rel')
        ->orderby('rel.mmw_sorting ASC')
        ->where('rel.<?php echo $relation['local'] ?> = ?', $this->object->getPrimaryKey())
        ->execute();

      $this->setDefault('<?php echo $this->underscore($relation['alias']) ?>_list', $defaults->getPrimaryKeys());
    }
<?php else: ?>
    if (isset($this->widgetSchema['<?php echo $this->underscore($relation['alias']) ?>_list']))
    {
...
<?php if ($relation->getAssociationTable()->hasField('mmw_sorting')): ?>
	...
	{
		$obj = new <?php echo $relation->getAssociationTable()->getComponentName(); ?>;
		$obj->set('<?php echo $relation['local']; ?>', $this->object->getPrimaryKey());
		$obj->set('<?php echo $relation['foreign']; ?>', $value);
		$obj->set('mmw_sorting', $i++);
		$obj->save();
	}
<?php else: ?>
